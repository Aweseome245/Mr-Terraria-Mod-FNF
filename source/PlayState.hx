...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'freedom':
				dialogue = CoolUtil.coolTextFile(Paths.txt('freedom/dialog'));
			case 'rap-for-another-night':
				dialogue = CoolUtil.coolTextFile(Paths.txt('rap-for-another-night/dialog'));
			case 'umad':
				dialogue = CoolUtil.coolTextFile(Paths.txt('umad/dialog'));
		}
		...
		switch(stageCheck)
		{
			...
			{
				defaultCamZoom = 0.7;
				curStage = 'terrariaRoom';
				var bg:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('terrariaRoom/bg'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
			...
			case 'tankBG':
				{
					defaultCamZoom = 0.7;
					curStage = 'tankBG';
					var bg:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('stressBG/tankbg'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'terraria':
					babyArrow.frames = Paths.getSparrowAtlas('terrariaNote/NOTE_assets');
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					//huge thanks to bbpanzu for making the Momi mod open source, really helped with this lmao
					if (SONG.song.toLowerCase() == 'umad')
					{
						Conductor.changeBPM(0);
						
						var dia = CoolUtil.coolTextFile(Paths.txt("umad/endDialog"));
						
						var endDiaBox:DialogueBox = new DialogueBox(false, dia);
						
						schoolIntro(endDiaBox);
						endDiaBox.cameras = [camHUD];
						endDiaBox.scrollFactor.set();
						endDiaBox.finishThing = endStuff;
					}
					else
					{
						FlxG.sound.playMusic(Paths.music('freakyMenu'));
				...
				else
				{
					...
					//Videostate that works, but the cutscene's audio is weirdly slow despite the correct amount of frames being used, if someone could DM me on discord username Aweseome245#7012 and help me with this that would help out a lot! :)

					/*
					switch(SONG.song.toLowerCase())
					{
						case 'umad':
							trace('Currently playing videostate');
							LoadingState.loadAndSwitchState(new VideoState("assets/videos/cutsceneGame/cutscene.webm", new PlayState()));
						default:
							LoadingState.loadAndSwitchState(new PlayState());
					}
					*/
